 ...
 * @return {bool}
 */
TileServer.prototype.hasLayer = function(name) {
 ...
 * @return {void}
 */
TileServer.prototype.removeLayer = function(name) {
	...
	for (var val in layer.routes) {
		route = layer.routes[val];

		if (route.handler.provider.plugin.sourc != undefined
			&& (typeof route.handler.provider.plugin.sourc === 'function')
			&& route.handler.provider.plugin.sourc() != undefined)
			route.handler.provider.plugin.sourc().close(function () {});
		route.handler._destroy(this, function () {});
		delete layer.routes[val];
	}
...
TileServer.prototype._handleRequest = function(req, res, next) {
	...
	var key = 'time=forestar';
	if (tilereq != undefined && tilereq.qs != undefined && tilereq.qs.indexOf(key) != -1)
	{
		var date = new Date();
		console.log('request [' + req.url + '] is start at ' + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + ":" + date.getMilliseconds());
		var start = Date.now();
	}
	...
	this.serve(tilereq, {req: req, res: res}, function(status, buffer, headers) {
		...
		if (tilereq != undefined && tilereq.qs != undefined && tilereq.qs.indexOf(key) != -1)
		{
            ...
            var date = new Date();
			console.log('request [' + req.url + '] is finish at ' + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds() + ":" + date.getMilliseconds() + ', total time is ' + dur + 'ms');
		}
	...
	async.series([
		...
		function finalizeResult(callback) {
			...
			if (result.status !== 204) {
				if ((result.headers['Transfer-Encoding'] == undefined || result.headers['Transfer-Encoding'] != 'chunked')
				&& (result.headers['transfer-encoding'] == undefined || result.headers['transfer-encoding'] != 'chunked'))
 ...
 * @return {void}
 */
TileServer.prototype.syncBalancerNodeLayers = function() {
	var self = this;
	var balancer = this.options.balancer;
	if (!balancer) return;
	if (!this.balancer) return;
	...
	request({
		json: true,
		method: 'POST',
		url: 'http://' + balancer.host + '/syncnodelayers',
		timeout: balancer.register_timeout || 30000,
		body: self._buildBalancerData()
	}, function(err, res, body) {
